...
function App() {
  ...
  // 1. Se escolheu "Entrar como Cliente", mostra o app de delivery
  if (modoCliente) {
    return <ClienteApp onLogout={() => setModoCliente(false)} />;
  }

  // 2. Se fez login com sucesso, mostra o painel de lojista
  if (usuarioLogado) {
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (min-width: 1600px) { .login-banner { flex: 1.5; } }
        ...
        <div className="login-banner">
          <div style={{ position: 'relative', zIndex: 10 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginTop: '3rem', maxWidth: '400px', marginLeft: 'auto', marginRight: 'auto' }}>
              {['🚀', '💼', '📊'].map((emoji, index) => (
                <div key={index} style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(5px)', padding: '1rem', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.2)' }}>
                  <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{emoji}</div>
                  <div style={{ fontSize: '0.875rem', fontWeight: '600' }}>
                    {['Agilidade', 'Segurança', 'Análises'][index]}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="form-wrapper">
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              ...
              <p style={{ margin: 0, color: '#64748b', fontSize: '1rem' }}>
                {isLogin ? 'Acesse o painel administrativo' : 'Crie sua conta corporativa'}
              </p>
            </div>
            ...
            {mensagem && (
              <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#fef2f2', color: '#991b1b', borderRadius: '1rem', fontSize: '0.875rem', textAlign: 'center', border: '1px solid #fecaca', animation: 'slideIn 0.3s ease' }}>
                ⚠️ {mensagem}
              </div>
            )}
            ...
            {/* BOTÃO PARA O CLIENTE FINAL ACESSAR O APP DE DELIVERY */}
            <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px dashed #e2e8f0', textAlign: 'center' }}>
              <p style={{ margin: '0 0 10px 0', fontSize: '0.9rem', color: '#64748b' }}>Apenas querendo pedir comida?</p>
              <button onClick={() => setModoCliente(true)} className="btn-cliente">
...
const inputStyle = { height: '3.5rem', width: '100%', padding: '0 1.25rem 0 3rem', borderRadius: '1rem', border: '2px solid #e2e8f0', fontSize: '1rem', outline: 'none', backgroundColor: '#ffffff', transition: 'all 0.3s', boxSizing: 'border-box' };
const btnPrimaryStyle = { height: '3.5rem', width: '100%', borderRadius: '1rem', border: 'none', background: 'linear-gradient(135deg, #dc2626, #b91c1c)', color: 'white', fontSize: '1rem', fontWeight: '700', cursor: 'pointer', transition: 'all 0.3s', boxShadow: '0 10px 20px -5px rgba(220, 38, 38, 0.4)' };
const btnGoogleStyle = { width: '100%', height: '3.5rem', borderRadius: '1rem', border: '2px solid #e2e8f0', backgroundColor: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', fontSize: '1rem', fontWeight: '600', color: '#1e293b', cursor: 'pointer', transition: 'all 0.3s' };
